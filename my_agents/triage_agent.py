...
def handle_handoff(
    ...
    with st.sidebar:
        st.write(f"""
            ...
            Description {input_data.issuse_description}
""")
...
triage_agent = Agent(
    ...
    # ]
    handoffs=[
        make_handoff(agent=technical_agent),
        make_handoff(agent=account_agent),
        make_handoff(agent=billing_agent),
        make_handoff(agent=order_agent),
    ]
)